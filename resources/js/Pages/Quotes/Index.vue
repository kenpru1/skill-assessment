...
const favQuote = (quote:any) => {
    loading = false

    let favorite = JSON.parse(quote).quote

    console.log(props)
    

    axios.post(API_URL + 'storeQuote', {favorite, "id": user_id }).then(response => {
                    ...
                    <div class="col-md-10 offset-md-1">
                        <div class="table-responsive">
                            <table class="table table-stripeted table-bordered">
                                <thead>
                                    <tr>
                                        <th>#</th><th>CITAS CELEBRES</th><th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr v-for="quote,index in quotes" :key="index">
                                        <td>{{ index+1 }}</td>
                                        <td>{{ JSON.parse(quote).quote }}</td>
                                        <td>
                                            <button class="btn btn-primary" @click="favQuote(quote)">
